---
title: "Delphos"
subtitle: "Świecący pierścień w glebie, która latami nie chłonęła wody"
dateDisplay: "2 listopada 1971"
countryName: "Stany Zjednoczone"
location: "Farma w pobliżu Delphos, hrabstwo Ottawa, Kansas"
witnesses: "Nastoletni chłopiec, następnie jego rodzice; badacze pobrali próbki z miejsca"
duration: "kilka minut"
summary: "Chłopiec z farmy zgłosił obiekt w kształcie grzyba, który zawisł i odleciał, zostawiając na ziemi świecący pierścień. Gleba w pierścieniu stała się silnie hydrofobowa, latami odpychała wodę i była analizowana przez kilka laboratoriów bez zgody co do przyczyny."
official: "Brak dochodzenia federalnego; Blue Book był już zamknięty. Sprawę badali naukowcy cywilni."
alternatives:
- "Pierścień grzybni, część grzybów glebowych wytwarza hydrofobową grzybnię i rośnie kołowo. Wyjaśnienie wiodące, kwestionowane ze względu na ostrość granicy i głębokość efektu."
- "Wyciek chemiczny lub pozostałości rolnicze."
- "Rzeczywista interakcja fizyczna z niezidentyfikowanym obiektem."
sources:
- label: "Analizy gleby wykonane przez kilka niezależnych laboratoriów, 1971–1975"
  note: "Analizy zgadzają się, że gleba była silnie hydrofobowa, i nie zgadzają co do przyczyny."
- label: "Ówczesne fotografie pierścienia"
- label: "Późniejsze przeglądy chemii gleby"
---

## Sprawa ze śladem fizycznym, na którą Amerykanie powinni wskazywać zamiast Roswell

Delphos ma to, czego Roswell nie ma: **materiał, który pobrano, przeanalizowano i zachowano.**

Gleba z pierścienia odpychała wodę tak silnie, że dawała się na niej unosić. Efekt utrzymywał się
latami i został udokumentowany przez więcej niż jedno laboratorium. Fotografie wykonano na bieżąco.

## Wyjaśnienie grzybnią, potraktowane serio

Niektóre grzyby glebowe tworzą pierścienie i wytwarzają hydrofobową grzybnię. To zjawisko realne,
częste, i jest wyjaśnieniem wiodącym.

Jego trudności są konkretne: raportowana ostrość granicy pierścienia, głębokość, na jaką sięgała
hydrofobowość, i raportowana luminescencja w momencie zdarzenia. Żadna z nich nie przesądza przeciwko
odczytowi grzybowemu i uczciwe potraktowanie sprawy musi to powiedzieć.

## Dlaczego stoi tam, gdzie stoi

Jeden młody świadek, brak radaru, brak obserwatora potwierdzającego w momencie zdarzenia. Ale
**ślad przeżył relację świadka o lata i mógł być mierzony przez obcych ludzi**, a to dokładnie ta
własność, która czyni Trans-en-Provence cennym, i dokładnie to, czego sprawy uprowadzeń nigdy nie
zaoferują.
